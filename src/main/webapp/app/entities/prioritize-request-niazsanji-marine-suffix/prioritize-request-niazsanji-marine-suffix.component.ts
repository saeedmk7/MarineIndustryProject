...
export class PrioritizeRequestNiazsanjiMarineSuffixComponent implements OnInit, OnDestroy {
    ...
    loadAll(criteria?, excelExport: boolean = false) {
        ...
        if (excelExport) {
            this.prioritizeRequestNiazsanjiService
                ...
                .subscribe(
                    (res: HttpResponse<IPrioritizeRequestNiazsanjiMarineSuffix[]>) => this.prepareForExportExcel(res.body),
    ...
    prepareResult() {
        this.prioritizeRequestNiazsanjis.forEach((a: IPrioritizeRequestNiazsanjiMarineSuffix) => {
            ...
            if (org) a.organizationChartRootTitle = org.fullTitle.split('>')[0];
        });
        1;
        this.totalHour = this.prioritizeRequestNiazsanjis
            .filter(a => a.totalLearningTime)
            .map(a => a.totalLearningTime)
            .reduce((sum, current) => sum + current);
        this.totalPriceCost = this.prioritizeRequestNiazsanjis
