...
export class LevelThreeEffectivenessMarineSuffixUpdateComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        this.activatedRoute.data.subscribe(({ levelThreeEffectiveness }) => {
            ...
            if(this.levelThreeEffectiveness.finalNiazsanjiReportPersonId){
                this.finalNiazsanjiReportPersonService.find(this.levelThreeEffectiveness.finalNiazsanjiReportPersonId)
                    .subscribe((resp: HttpResponse<IFinalNiazsanjiReportPersonMarineSuffix>) => {
                        ...
                        this.levelThreeEffectivenessService.query({
                            page: 0,
                            size: 20000,
                            ...
                            (res: HttpResponse<ILevelThreeEffectivenessMarineSuffix[]>) => {
                                if(res.body && res.body.length > 0)
                                    this.levelThreeEffectiveness = res.body[0];
                                this.getScores();
    ...
    getScores(){
        if(this.levelThreeEffectiveness.id !== undefined){
            ...
            this.levelThreeScoreService.query({
                page: 0,
                size: 20000,
                criteria,
                sort: ["id", "asc"]
    ...
    buildLevelThreeEffectivenessScores(levelThreeScores: ILevelThreeScoreMarineSuffix[], gradeId){
        ...
        this.levelThreeCriteriaService.query({
            page: 0,
            size: 20000,
            criteria,
            sort: ["id", "asc"]
            ...
            (res: HttpResponse<ILevelThreeCriteriaMarineSuffix[]>) => {
                this.levelThreeCriterias = res.body.sort((a,b) => ((a.levelThreeCriteriaGroupId > b.levelThreeCriteriaGroupId) ? 1 : (a.levelThreeCriteriaGroupId < b.levelThreeCriteriaGroupId) ? -1 : 0) || ((a.displayOrder > b.displayOrder) ? 1 : (a.displayOrder < b.displayOrder) ? -1 : 0));
    ...
    sumScores(){
        const totalScore = this.levelThreeEffectiveness.levelThreeScores
            .map(a => a.score * a.levelThreeCriteriaWeight).reduce((sum, current) => sum + current);
        this.levelThreeEffectiveness.totalScore = totalScore;
        const totalWeight = this.levelThreeEffectiveness.levelThreeScores
            .map(a => a.levelThreeCriteriaWeight).reduce((sum, current) => sum + current);
