...
export class NiazsanjiPersonGradeMarineSuffixUpdateComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        this.activatedRoute.data.subscribe(({ niazsanjiPersonGrade }) => {
            ...
            if(this.niazsanjiPersonGrade.finalNiazsanjiReportPersonId){
                this.finalNiazsanjiReportPersonService.find(this.niazsanjiPersonGrade.finalNiazsanjiReportPersonId)
                    .subscribe((resp: HttpResponse<IFinalNiazsanjiReportPersonMarineSuffix>) => {
                       ...
                       this.finalNiazsanjiReport = this.finalNiazsanjiReportPerson.finalNiazsanjiReport;
                        ...
                        this.niazsanjiPersonGradeService.query({
                            page: 0,
                            size: 20000,
                            ...
                            (res: HttpResponse<INiazsanjiPersonGradeMarineSuffix[]>) => {
                                if(res.body && res.body.length > 0)
                                    this.niazsanjiPersonGrade = res.body[0];
                                this.getScores();
    ...
    getScores(){
        if(this.niazsanjiPersonGrade.id !== undefined){
            ...
            this.niazsanjiPersonGradeScoreService.query({
                page: 0,
                size: 20000,
                criteria,
                sort: ["id", "asc"]
    ...
    buildNiazsanjiPersonGradeScores(niazsanjiPersonGradeScores: INiazsanjiPersonGradeScoreMarineSuffix[], gradeId){
        this.niazsanjiPersonCriteriaService.query().subscribe(
            (res: HttpResponse<INiazsanjiPersonCriteriaMarineSuffix[]>) => {
                this.niazsanjiPersonCriterias = res.body.sort((a,b) => ((a.criterionType > b.criterionType) ? 1 : (a.criterionType < b.criterionType) ? -1 : 0) || ((a.displayOrder > b.displayOrder) ? 1 : (a.displayOrder < b.displayOrder) ? -1 : 0));
    ...
    sumScores(){
        const totalScore = this.niazsanjiPersonGrade.niazsanjiPersonGradeScores.map(a => a.score * a.niazsanjiPersonCriteriaWeight).reduce((sum, current) => sum + current);
        this.niazsanjiPersonGrade.totalScore = totalScore;
        const totalWeight = this.niazsanjiPersonGrade.niazsanjiPersonGradeScores.map(a => a.niazsanjiPersonCriteriaWeight).reduce((sum, current) => sum + current);
