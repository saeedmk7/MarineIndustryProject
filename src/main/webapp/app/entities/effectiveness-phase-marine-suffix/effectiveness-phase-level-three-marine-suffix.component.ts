    ...
    loadData() {
        this.finalNiazsanjiReportPersonService
            ...
            .subscribe((resp: HttpResponse<IFinalNiazsanjiReportPersonMarineSuffix[]>) => {
                ...
                this.fullAverage = presentPeople.map(w => w.levelThreeScore).reduce((sum, current) => sum + current) / presentPeople.length;
                this.fullGrade = this.convertObjectDatesService.calculateGrade(this.fullAverage);
                if (presentPeople.filter(w => w.levelThreeScore > 0).length == presentPeople.length) {
