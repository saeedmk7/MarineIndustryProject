...
@Component({
    selector: 'mi-soldier-media-awareness-report-marine-suffix-update',
    templateUrl: './soldier-media-awareness-report-marine-suffix-update.component.html'
})
export class SoldierMediaAwarenessReportMarineSuffixUpdateComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        this.activatedRoute.data.subscribe(({ soldierMediaAwarenessReport }) => {
            this.soldierMediaAwarenessReport = soldierMediaAwarenessReport;
        });
    ...
    uploadFile() {
        ...
        this.soldierMediaAwarenessReportService.uploadFile(formdata).subscribe(
            event => {
                if (event instanceof HttpResponse) {
                    if (event.body) {
                        this.soldierMediaAwarenessReport.fileDoc = event.body.toString();
    ...
    save() {
        ...
        this.soldierMediaAwarenessReport.reportTime = 0;
        if (this.soldierMediaAwarenessReport.id !== undefined) {
            this.subscribeToSaveResponse(this.soldierMediaAwarenessReportService.update(this.soldierMediaAwarenessReport));
        } else {
            if (this.file) {
                let formdata: FormData = new FormData();
                ...
                this.soldierMediaAwarenessReportService.uploadFile(formdata).subscribe(
                    event => {
                        if (event.type === HttpEventType.UploadProgress) {
                            ...
                            if (event.body) {
                                this.soldierMediaAwarenessReport.fileDoc = event.body;
                                this.subscribeToSaveResponse(
                                    this.soldierMediaAwarenessReportService.create(this.soldierMediaAwarenessReport)
                                );
                            }
                        }
                    },
                ...
                this.subscribeToSaveResponse(this.soldierMediaAwarenessReportService.create(this.soldierMediaAwarenessReport));
            }
        }
    }

    protected subscribeToSaveResponse(result: Observable<HttpResponse<ISoldierMediaAwarenessReportMarineSuffix>>) {
        result.subscribe(
            (res: HttpResponse<ISoldierMediaAwarenessReportMarineSuffix>) => this.onSaveSuccess(),
