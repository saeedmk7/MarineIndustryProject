...
export class EducationalHistoryMarineSuffixComponent implements OnInit, OnDestroy {
    ...
    loadAll(criteria?, excelExport: boolean = false) {
        ...
        const requestStatusFilter = criteria.find(a => a.key == 'requestStatus.equals');
        if (requestStatusFilter) {
            let val = requestStatusFilter.value;
            if (val && val == 'NEW') {
                criteria = criteria.filter(a => a.key != 'status.in' || a.key != 'status.equals');
                criteria = criteria.filter(a => a.key != 'requestStatus.equals');

                criteria.push({
                    key: 'status.equals',
                    value: this.currentPerson.organizationChartId
                });
            }
        }
        if (excelExport) {
                ...
                this.courseTypeService.query().subscribe(
                    (res: HttpResponse<ICourseTypeMarineSuffix[]>) => {
                        ...
                        this.searchbarModel.push(
                            new SearchPanelModel('educationalHistory', 'courseTypeId', 'select', 'equals', this.courseTypes)
                        );
                    },
                    (res: HttpErrorResponse) => this.onError(res.message)
                );
                let status = [{ id: 'ACCEPT', title: 'تایید شده' }, { id: 'IGNORE', title: 'رد شده' }, { id: 'NEW', title: 'منتظر تایید' }];
                this.searchbarModel.push(
                    new SearchPanelModel('educationalHistory', 'requestStatus', 'selectWithStringId', 'equals', status)
                );
            });
        });
        ...
        this.personService
            ...
            .subscribe(
                (resp: HttpResponse<IPersonMarineSuffix[]>) => {
                    this.searchbarModel.push(
                        new SearchPanelModel('educationalHistory', 'personId', 'select', 'equals', resp.body, 'fullName', '')
                    );
                },
    ...
    prepareSearchOrgChart() {
        if (this.organizationChartService.organizationchartsAll) {
            this.organizationcharts = this.organizationChartService.organizationchartsAll;
            if (this.isSuperUsers) {
                this.groups = this.organizationcharts
                    .filter(a => a.parentId == null)
                    .sort((a, b) => (a.id > b.id ? 1 : a.id < b.id ? -1 : 0));
                this.searchbarModel.push(
                    new SearchPanelModel('educationalHistory', 'organizationChartId', 'select', 'equals', this.groups, 'fullTitle', 'half')
                );
            }
    ...
    handleOrgChartView(): IOrganizationChartMarineSuffix[] {
        if (this.isAdmin) return this.organizationcharts;
