...
export class EducationalHistoryMarineSuffixUpdateComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        this.activatedRoute.data.subscribe(({ educationalHistory }) => {
            ...
            if (this.educationalHistory.id !== undefined) this.onPersonChange({ id: this.educationalHistory.personId });
        });
    ...
    onEducationalModuleChange($event: IEducationalModuleMarineSuffix) {
        ...
        this.educationalHistory.totalTime =
            (this.educationalHistory.learningTimeTheorical ? +this.educationalHistory.learningTimeTheorical : 0) +
            (this.educationalHistory.learningTimePractical ? +this.educationalHistory.learningTimePractical : 0);
        ...
        if (organization.parentId > 0) {
            ...
            this.targetPeople.push(
                new PersonMarineSuffix(
                    ...
                    'ثبت نهایی',
                    'ثبت نهایی',
                    'پرونده/سابقه آموزشی شما پس از ثبت برای کارشناس ارشد آموزش سازمان برای بازبینی ارسال می شود.'
                )
            );
        }
    }
    ...
    uploadFile() {
        ...
        this.educationalHistoryService.uploadFile(formdata).subscribe(
            event => {
    ...
    save() {
        ...
        if (this.educationalHistory.id !== undefined) {
            ...
            if (!this.educationalHistory.personId) this.educationalHistory.personId = this.currentPerson.id;
            ...
            if (org.parentId > 0) this.educationalHistory.status = org.parentId;
            else this.educationalHistory.status = 0;
        ...
        if (!isEdit) {
            if (res.status == 0) {
                ...
                this.educationalHistoryService
                    .finalize(res)
                    .subscribe((res: HttpResponse<IEducationalHistoryMarineSuffix>) => res, (res: HttpErrorResponse) => res);
            }
        }
        this.isSaving = false;
        this.previousState();
    ...
    checkDateOfEndValidation(event) {
        try {
            if (persianMoment(event.target.value, 'jYYYY/jMM/jDD').isValid()) {
                this.dateOfEndValid = 1;
            } else {
                this.dateOfEndValid = 2;
            }
        } catch (e) {
            this.dateOfEndValid = 2;
        }
