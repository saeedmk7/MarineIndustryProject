...
export class EducationalModuleMarineSuffixComponent implements OnInit, OnDestroy {
    ...
    prepareForExportExcel(res: IEducationalModuleMarineSuffix[]) {
        ...
        res.forEach(a => {
            ...
            obj = {
                ...
                peopleUnderTraining:
                    a.peopleUnderTrainings && a.peopleUnderTrainings.length > 0 ? a.peopleUnderTrainings.map(w => w.title).join(' - ') : '',
                ...
                totalLearningTime:
                    (a.learningTimeTheorical ? a.learningTimeTheorical : 0) + (a.learningTimePractical ? a.learningTimePractical : 0),
                ...
                teachingApproach:
                    a.teachingApproaches && a.teachingApproaches.length > 0 ? a.teachingApproaches.map(w => w.title).join(' - ') : ' ',
                ...
                educationalCenter:
                    a.educationalCenters && a.educationalCenters.length > 0 ? a.educationalCenters.map(a => a.name).join(' - ') : ' ',
                teachersText: a.teachersText,
                teacher: a.teachers && a.teachers.length ? a.teachers.map(w => w.fullName).join(' - ') : ' ',
                effectivenessLevel:
                    a.effectivenessLevels && a.effectivenessLevels.length > 0 ? a.effectivenessLevels.map(w => w.title).join(' - ') : ' ',
                evaluationMethod: a.evaluationMethodTitle,
                assessmentMethod:
                    a.assessmentMethods && a.assessmentMethods.length > 0 ? a.assessmentMethods.map(w => w.title).join(' - ') : ' ',
                effectivenessIndex:
                    a.effectivenessIndices && a.effectivenessIndices.length > 0
                        ? a.effectivenessIndices.map(w => w.title).join(' - ')
                        : ' ',
    ...
    setRoles(account: any) {
        ...
        if (this.isKarshenasArshadAmozeshSazman || this.isModirKolAmozesh || this.isAdmin) this.isTopUsers = true;
    }
    prepareOrganization() {
    ...
    prepareScientific() {
        this.scientificWorkGroupService.query().subscribe((res: HttpResponse<IScientificWorkGroupMarineSuffix[]>) => {
            this.scientificWorkGroups = res.body;
            this.searchbarModel.push(
                new SearchPanelModel('educationalModule', 'scientificWorkGroupId', 'select', 'equals', this.scientificWorkGroups)
            );
        }),
            (res: HttpErrorResponse) => this.onError(res.message);
    }
    prepareSkillableLevelOfSkill() {
        this.skillableLevelOfSkillService.query().subscribe((res: HttpResponse<ISkillableLevelOfSkillMarineSuffix[]>) => {
            this.skillableLevelOfSkills = res.body;
            this.searchbarModel.push(
                new SearchPanelModel('educationalModule', 'skillableLevelOfSkillId', 'select', 'equals', this.skillableLevelOfSkills)
            );
        }),
            (res: HttpErrorResponse) => this.onError(res.message);
    }
