...
import { ITeachApproachMarineSuffix, TeachApproachMarineSuffix } from 'app/shared/model/teach-approach-marine-suffix.model';
...
export class RequestEducationalModuleMarineSuffixUpdateComponent implements OnInit {
    ...
    showHeadlines(id) {
        ...
        this.headlineService
            ...
            .subscribe(
                (resp: HttpResponse<IHeadlineMarineSuffix[]>) => {
        ...
        if (organization.parentId > 0) {
            ...
            this.targetPeople.push(
                new PersonMarineSuffix(
                    ...
                    'ثبت نهایی',
                    'ثبت نهایی',
                    'درخواست طراحی شما پس از ثبت برای کارشناس ارشد آموزش سازمان برای بازبینی ارسال می شود.'
                )
            );
        }
    }
    ...
    save() {
        ...
        if (this.requestEducationalModule.recommendDatePersian) {
            this.requestEducationalModule.recommendDate = this.convertObjectDatesService.shamsi2miladiMoment(
                this.requestEducationalModule.recommendDatePersian
            );
        }
        if (this.requestEducationalModule.timePassedPersian) {
            this.requestEducationalModule.timePassed = this.convertObjectDatesService.shamsi2miladiMoment(
                this.requestEducationalModule.timePassedPersian
            );
        }
        if (this.requestEducationalModule.creditPersian) {
            this.requestEducationalModule.credit = this.convertObjectDatesService.shamsi2miladiMoment(
                this.requestEducationalModule.creditPersian
            );
        }
        ...
        this.requestEducationalModule.headlines.forEach(a => {
            ...
            if (headlineValue) a.headlineLevel = headlineValue;

            name = 'headlineScope_' + a.id;
            radio = 'input[name=' + name + ']:checked';
            headlineValue = $(radio).val();
            if (headlineValue) a.headlineScope = headlineValue;
        ...
        this.requestEducationalModule.learningTimePractical = this.requestEducationalModule.learningTimePractical
            ? this.requestEducationalModule.learningTimePractical
            : 0;
        this.requestEducationalModule.learningTimeTheorical = this.requestEducationalModule.learningTimeTheorical
            ? this.requestEducationalModule.learningTimeTheorical
            : 0;
        this.message = '';
