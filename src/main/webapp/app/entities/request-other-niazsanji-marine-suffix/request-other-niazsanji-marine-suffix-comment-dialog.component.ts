...
import {RequestOtherNiazsanjiMarineSuffixService} from "app/entities/request-other-niazsanji-marine-suffix/request-other-niazsanji-marine-suffix.service";

@Component({
    selector: 'mi-request-other-niazsanji-marine-suffix-comment-dialog',
    templateUrl: './request-other-niazsanji-marine-suffix-comment-dialog.component.html'
})
export class RequestOtherNiazsanjiMarineSuffixCommentDialogComponent implements OnInit {
    ...
    confirmDelete(id: number) {
        this.requestOtherNiazsanjiService.delete(id).subscribe(response => {
            this.eventManager.broadcast({
                ...
                content: 'Commented an requestOtherNiazsanji'
            });
    ...
    save(){
        ...
        switch (this.commentType) {
            case 'COMMENT':
                ...
                this.requestOtherNiazsanji.conversation += " ثبت نظر توسط " + currentUserFullName + " در تاریخ: " + this.convertObjectDatesService.miladi2Shamsi(new Date()) + " ثبت شد. ";
                this.requestOtherNiazsanji.conversation += "\n";
                this.requestOtherNiazsanji.conversation += currentUserFullName + ": " + this.comment;
                break;
            case 'REJECT':
                if(!this.comment)
                {
                ...
                this.requestOtherNiazsanji.conversation += " رد درخواست توسط " + currentUserFullName + " در تاریخ: " + this.convertObjectDatesService.miladi2Shamsi(new Date()) + " انجام شد. ";
                this.requestOtherNiazsanji.conversation += "\n";
                this.requestOtherNiazsanji.conversation += currentUserFullName + ": " + this.comment;

                this.requestOtherNiazsanji.requestStatus = RequestStatus.IGNORE;
                this.requestOtherNiazsanji.changeStatusUserLogin = this.currentAccount.login;
                break;
            case 'ACCEPT':
                this.requestOtherNiazsanji.conversation += " تایید درخواست توسط " + currentUserFullName + " در تاریخ: " + this.convertObjectDatesService.miladi2Shamsi(new Date()) + " انجام شد. ";
                if(this.comment) {
                    this.requestOtherNiazsanji.conversation += "\n";
                    this.requestOtherNiazsanji.conversation += currentUserFullName + ": " + this.comment;
                }
                ...
                if(organization.parentId > 0) {
                    this.requestOtherNiazsanji.status = organization.parentId;
                    this.requestOtherNiazsanji.changeStatusUserLogin = this.currentAccount.login;
                }
                else {
                    this.requestOtherNiazsanji.status = 0;
                    this.requestOtherNiazsanji.changeStatusUserLogin = this.currentAccount.login;
                    this.requestOtherNiazsanjiService.finalize(this.requestOtherNiazsanji).subscribe(
        ...
        this.requestOtherNiazsanjiService.update(this.requestOtherNiazsanji).subscribe(
            (res: HttpResponse<IFinalOrganizationNiazsanjiMarineSuffix>) => this.onSaveSuccess(),
            (res: HttpErrorResponse) => this.onSaveError(res)
        );
    }
    ...
    knowAboutStatusForContinue(){
        ...
        if(this.requestOtherNiazsanji.requestStatus == RequestStatus.ACCEPT || this.requestOtherNiazsanji.requestStatus == RequestStatus.IGNORE){
            this.clear();
        }
        if(this.requestOtherNiazsanji.status != this.currentPerson.organizationChartId){
            this.clear();
        }
    }
    ngOnInit(): void {
...
@Component({
    selector: 'mi-request-other-niazsanji-marine-suffix-comment-popup',
    template: ''
})
export class RequestOtherNiazsanjiMarineSuffixCommentPopupComponent implements OnInit, OnDestroy {
        ...
        this.activatedRoute.data.subscribe(({ requestOtherNiazsanji }) => {
            this.activatedRoute.params.subscribe((params) => {
                ...
                setTimeout(() => {
                    ...
                    this.ngbModalRef = this.modalService.open(RequestOtherNiazsanjiMarineSuffixCommentDialogComponent as Component, {
                        size: 'lg',
                        backdrop: false
                    });
